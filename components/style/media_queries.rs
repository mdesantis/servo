...
impl MediaQuery {
    fn parse(input: &mut Parser) -> Result<MediaQuery, ()> {
        ...
        let media_type = if let Ok(ident) = input.expect_ident() {
        ...
        while let Ok(token) = input.next() {
            // Each expression must begin with 'and'
            match token {
                Token::Ident(ref value) if value.eq_ignore_ascii_case("and") => {
                    expressions.push(try!(Expression::parse(input)))
                }
                _ => return Err(())
            }
