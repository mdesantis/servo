...
fn parse_one_src(context: &ParserContext, input: &mut Parser) -> Result<Source, ()> {
    let url = match input.next() {
        ...
        Ok(Token::Function(name)) => {
            if name.eq_ignore_ascii_case("local") {
                return Ok(Source::Local(try!(input.parse_nested_block().parse_entirely(|input| {
    ...
    let format_hints = match input.next() {
        Ok(Token::Function(name)) => {
            if !name.eq_ignore_ascii_case("format") {
                return Err(())
            }
            try!(input.parse_nested_block().parse_comma_separated(|input| {
