...
pub mod specified {
    ...
    impl LengthOrPercentage {
        fn parse_internal(input: &mut Parser, negative_ok: bool)
            ...
            match try!(input.next()) {
                ...
                Token::Percentage(ref value) if negative_ok || value.value >= 0. => {
        ...
        pub fn parse(input: &mut Parser) -> Result<PositionComponent, ()> {
            match try!(input.next()) {
                ...
                Token::Percentage(ref value) => {
                    Ok(PositionComponent::Percentage(value.value / 100.))
                }
                Token::Number(ref value) if value.value == 0. => {
    ...
    impl Angle {
        ...
        fn parse(input: &mut Parser) -> Result<Angle, ()> {
            match try!(input.next()) {
                        ...
                        "linear-gradient" => {
                            Ok(Image::LinearGradient(try!(
                                input.parse_nested_block()
                                     .parse_entirely(LinearGradient::parse_function))))
                        }
                        _ => Err(())
                    }
                }
